...
\begin{prop}
	Ensemble spaces are $\mathsf{T}_1$ topological spaces.
\end{prop}

\begin{proof}
	Since $MS(\ens[a], \ens) = 0$ if and only if $\ens = \ens[a]$, $\{\ens \in \Ens \, | \, MS(\ens[a], \ens) = 0\} = \{\ens[a]\}$. Since $MS$ is continuous, $\{\ens[a]\}$ is closed set. Since every singleton is closed, the topology is $\mathsf{T}_1$.
\end{proof}
...
\begin{remark}
	...
	Whether this is physically relevant or not depends on whether there are additional physical constraints on the mixing entropy between three points, which is very likely. The convexity of the entropic balls may, for example, help us show that the topology they generate is Hausdorff.
\end{remark}
